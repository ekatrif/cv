...
(self["webpackChunkcv_portfolio"] = self["webpackChunkcv_portfolio"] || []).push([ [ 561 ], {
    561: (__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
  ...
  */        function createElement(className, tagName, appendToEl) {
        ...
        class DOMEvents {
            ...
            _toggleListener(target, type, listener, passive, unbind, skipPool) {
                ...
                types.forEach((eType => {
                    if (eType) {
                        if (!skipPool) if (unbind) this._pool = this._pool.filter((poolItem => poolItem.type !== eType || poolItem.listener !== listener || poolItem.target !== target)); else this._pool.push({
                            target,
                            type: eType,
                            listener,
                            passive
        ...
        class PanBounds {
            ...
            update(currZoomLevel) {
                ...
                if (!this.slide.width) this.reset(); else {
                    this._updateAxis("x");
                    this._updateAxis("y");
        ...
        class Slide {
            ...
            setIsActive(isActive) {
                if (isActive && !this.isActive) this.activate(); else if (!isActive && this.isActive) this.deactivate();
            }
            ...
            zoomTo(destZoomLevel, centerPoint, transitionDuration, ignoreBounds) {
                ...
                if (!transitionDuration) finishTransition(); else pswp.animations.startTransition({
                    isPan: true,
                    name: "zoomTo",
        ...
        const PAN_END_FRICTION = .35;
        const VERTICAL_DRAG_FRICTION = .6;
        const MIN_RATIO_TO_CLOSE = .4;
        const MIN_NEXT_SLIDE_SPEED = .5;
        ...
        class DragHandler {
            ...
            change() {
                ...
                if (currSlide && currSlide.currZoomLevel > currSlide.zoomLevels.max || this.gestures.isMultitouch) this.gestures.zoomLevels.correctZoomPan(true); else {
        ...
        const UPPER_ZOOM_FRICTION = .05;
        const LOWER_ZOOM_FRICTION = .15;
        ...
        class ZoomHandler {
            ...
            change() {
                ...
                } else currZoomLevel = minZoomLevel - (minZoomLevel - currZoomLevel) * LOWER_ZOOM_FRICTION; else if (currZoomLevel > maxZoomLevel) currZoomLevel = maxZoomLevel + (currZoomLevel - maxZoomLevel) * UPPER_ZOOM_FRICTION;
                ...
                const {currSlide} = pswp;
                if ((!currSlide || currSlide.currZoomLevel < currSlide.zoomLevels.initial) && !this._wasOverFitZoomLevel && pswp.options.pinchToClose) pswp.close(); else this.correctZoomPan();
            }
            ...
            correctZoomPan(ignoreGesture) {
                ...
                if (prevZoomLevel < currSlide.zoomLevels.initial) destinationZoomLevel = currSlide.zoomLevels.initial; else if (prevZoomLevel > currSlide.zoomLevels.max) destinationZoomLevel = currSlide.zoomLevels.max; else {
        ...
        class TapHandler {
            ...
            click(point, originalEvent) {
                ...
                if (isImageClick) this._doClickOrTapAction("imageClick", point, originalEvent); else if (isBackgroundClick) this._doClickOrTapAction("bgClick", point, originalEvent);
            }
            ...
            _doClickOrTapAction(actionName, point, originalEvent) {
                ...
                switch (optionValue) {
                  ...
                  case "zoom-or-close":
                    if (currSlide !== null && currSlide !== void 0 && currSlide.isZoomable() && currSlide.zoomLevels.secondary !== currSlide.zoomLevels.initial) currSlide.toggleZoom(point); else if (pswp.options.clickToCloseNonZoomable) pswp.close();
                    break;
        ...
        class Gestures {
            constructor(pswp) {
                ...
                pswp.on("bindEvents", (() => {
                    pswp.events.add(pswp.scrollWrap, "click", this._onClick.bind(this));
                    if (this._pointerEventEnabled) this._bindEvents("pointer", "down", "up", "cancel"); else if (this._touchEventEnabled) {
            ...
            onPointerUp(e) {
                ...
                if (this._numActivePoints === 0) {
                    this._rafStopLoop();
                    if (this.isDragging) this._finishDrag(); else if (!this.isZooming && !this.isMultitouch) this._finishTap(e);
                }
            ...
            _updatePoints(e, pointerType) {
                if (this._pointerEventEnabled) {
                    ...
                    const pointerIndex = this._ongoingPointers.findIndex((ongoingPointer => ongoingPointer.id === pointerEvent.pointerId));
                    if (pointerType === "up" && pointerIndex > -1) this._ongoingPointers.splice(pointerIndex, 1); else if (pointerType === "down" && pointerIndex === -1) this._ongoingPointers.push(this._convertEventPosToPoint(pointerEvent, {
                        x: 0,
                        y: 0
                    ...
                    if (touchEvent.type.indexOf("touch") > -1) {
                        ...
                        if (pointerType === "up") this._numActivePoints = 0; else this._numActivePoints++;
                    }
                }
            }
            _updatePrevPoints() {
            ...
            _convertEventPosToPoint(e, p) {
                ...
                if ("pointerId" in e) p.id = e.pointerId; else if (e.identifier !== void 0) p.id = e.identifier;
                return p;
            }
            _onClick(e) {
        ...
        const MAIN_SCROLL_END_FRICTION = .35;
        class MainScroll {
            constructor(pswp) {
                this.pswp = pswp;
                this.x = 0;
            ...
            moveIndexBy(diff, animate, velocityX) {
                ...
                if (pswp.canLoop()) {
                    ...
                    if (distance <= numSlides / 2) diff = distance; else diff = distance - numSlides;
                } else {
                    if (newIndex < 0) newIndex = 0; else if (newIndex >= numSlides) newIndex = numSlides - 1;
        ...
        const getKeyboardEventKey = (key, isKeySupported) => isKeySupported ? key : KeyboardKeyCodesMap[key];
        class Keyboard {
            constructor(pswp) {
                this.pswp = pswp;
            ...
            _onKeyDown(e) {
                ...
                if (axis) {
                    e.preventDefault();
                    const {currSlide} = pswp;
                    if (pswp.options.arrowKeys && axis === "x" && pswp.getNumItems() > 1) keydownAction = isForward ? "next" : "prev"; else if (currSlide && currSlide.currZoomLevel > currSlide.zoomLevels.fit) {
            ...
            _onFocusIn(e) {
                ...
                if (template && document !== e.target && template !== e.target && !template.contains(e.target)) template.focus();
            }
        }
        ...
        const DEFAULT_DAMPING_RATIO = .75;
        class SpringEaser {
        ...
        class Animations {
            ...
            _start(props, isSpring) {
                const animation = isSpring ? new SpringAnimation(props) : new CSSAnimation(props);
            ...
            _onWheel(e) {
                ...
                if (e.ctrlKey || this.pswp.options.wheelToZoom) {
                    if (currSlide.isZoomable()) {
                        ...
                        if (e.deltaMode === 1) zoomFactor *= .05; else zoomFactor *= e.deltaMode ? 1 : .002;
        ...
        class UIElement {
            constructor(pswp, data) {
                ...
                if (data.onClick) element.onclick = e => {
                    if (typeof data.onClick === "string") pswp[data.onClick](); else if (typeof data.onClick === "function") data.onClick(e, element, pswp);
                };
        ...
        function initArrowButton(element, pswp, isNextButton) {
            ...
            pswp.on("change", (() => {
                if (!pswp.options.loop) if (isNextButton) element.disabled = !(pswp.currIndex < pswp.getNumItems() - 1); else element.disabled = !(pswp.currIndex > 0);
            }));
        }
        const arrowPrev = {
            name: "arrowPrev",
        ...
        class UI {
            ...
            init() {
                ...
                this.uiElementsData.sort(((a, b) => (a.order || 0) - (b.order || 0)));
                this.items = [];
            ...
            registerElement(elementData) {
                if (this.isRegistered) this.items.push(new UIElement(this.pswp, elementData)); else this.uiElementsData.push(elementData);
            }
            _onZoomPanUpdate() {
        ...
        function getThumbBounds(index, itemData, instance) {
            ...
            if (thumbnail) if (!itemData.thumbCropped) thumbBounds = getBoundsByElement(thumbnail); else thumbBounds = getCroppedBoundsByElement(thumbnail, itemData.width || itemData.w || 0, itemData.height || itemData.h || 0);
        ...
        class Opener {
            ...
            _applyStartProps() {
                ...
                if (options.showHideAnimationType === "fade") {
                    ...
                    this._duration = 0;
                    this._thumbBounds = void 0;
                } else if (this.isOpening && pswp._initialThumbBounds) this._thumbBounds = pswp._initialThumbBounds; else this._thumbBounds = this.pswp.getThumbBounds();
                ...
                if (this.isOpening) {
                    ...
                    if (this._croppedZoom) if (pswp.mainScroll.x !== 0) {
            ...
            _start() {
                ...
                })).finally((() => this._initiate())); else this._initiate();
            }
            _initiate() {
            ...
            _onAnimationComplete() {
                ...
                if (this.isClosed) pswp.destroy(); else if (this.isOpen) {
                    var _pswp$currSlide;
